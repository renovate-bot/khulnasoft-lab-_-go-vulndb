...
import (
	...
	"github.com/khulnasoft-lab/go-vulndb/internal"
	...
	"github.com/khulnasoft-lab/go-vulndb/internal/worker"
)

var (
	githubToken = flag.String("ghtoken", os.Getenv("VULN_GITHUB_ACCESS_TOKEN"), "GitHub access token")
	issueRepo   = flag.String("issue-repo", "github.com/khulnasoft-lab/go-vulndb", "repo to create issues in")
)
...
func constructIssue(ctx context.Context, c *issues.Client, ghsaClient *ghsa.Client, pc *proxy.Client, alias string, labels []string) (err error) {
	...
	repo, err := gitrepo.Clone(ctx, "https://github.com/khulnasoft-lab/go-vulndb")
	if err != nil {
		return err
	}
	...
	fmt.Printf("created https://github.com/khulnasoft-lab/go-vulndb/issues/%d (%s)\n", issNum, strings.Join(ids, ", "))
	return nil
}
