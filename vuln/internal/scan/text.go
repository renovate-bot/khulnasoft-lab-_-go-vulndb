...
import (
	...
	"github.com/khulnasoft-lab/go-vulndb/vuln/internal"
	"github.com/khulnasoft-lab/go-vulndb/vuln/internal/govulncheck"
	"github.com/khulnasoft-lab/go-vulndb/vuln/internal/osv"
)
...
const (
	detailsMessage = `For details, see https://pkg.go.dev/github.com/khulnasoft-lab/go-vulndb/vuln/cmd/govulncheck.`
...
func (h *TextHandler) byVulnerability(findings []*findingSummary) {
	...
	h.print("See https://pkg.go.dev/github.com/khulnasoft-lab/go-vulndb/vuln/cmd/govulncheck for details.\n\n")
	index := 0
	for _, findings := range byVuln {
		if !isCalled(findings) {
